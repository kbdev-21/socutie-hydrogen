  ...
  return (
    <div>
      {compareAtPrice && Number(compareAtPrice.amount) > Number(price.amount) ? (
        ...
        <div className={`${size === "normal" ? "text-lg" : "text-base"} font-normal text-light-text1`}>{formatVnd(price.amount)}{price.currencyCode === "VND" ? "₫" : "$"}</div>
      ) : (
        <span>&nbsp;</span>
      )}
    </div>
  ...
  function SalePriceDisplay() {
    return (
      <div className={`flex ${size === "normal" ? "gap-2" : "gap-1"} items-center`}>
        <div className={`${size === "normal" ? "text-lg" : "text-base"} font-[500] text-light-secondary`}>{formatVnd(price!.amount)}{price!.currencyCode === "VND" ? "₫" : "$"}</div>
